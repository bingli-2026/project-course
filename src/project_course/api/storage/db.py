...
CREATE INDEX IF NOT EXISTS idx_window_samples_task
    ON window_samples(task_id, window_index);
    ...
    with connect() as conn:
        row = conn.execute(
            ...
            SELECT
              ...
              SUM(
                  CASE WHEN task_status IN ('succeeded','failed')
                  THEN 1 ELSE 0 END
              ) AS total
            FROM tasks
...
def list_windows(task_id: str) -> list[dict[str, Any]]:
    with connect() as conn:
        rows = conn.execute(
            "SELECT payload_json FROM window_samples "
            "WHERE task_id = ? ORDER BY window_index",
            (task_id,),
        ).fetchall()
...
def get_window(task_id: str, window_index: int) -> dict[str, Any] | None:
    with connect() as conn:
        row = conn.execute(
            "SELECT payload_json FROM window_samples "
            "WHERE task_id = ? AND window_index = ?",

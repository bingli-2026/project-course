...
def _synth_window(window_index: int, task_id: str, profile_name: str) -> dict:
    ...
    return {
        ...
        "center_time_s": round(
            window_index * settings.window_hop_s + settings.window_size_s / 2,
            4,
        ),
        ...
        "vision_dy_peak_power": round(
            profile["power"] * 1.05 + rng.normal(0, 0.05),
            4,
        ),
        ...
        "sensor_az_peak_hz": round(
            profile["sensor_az"] + rng.normal(0, j * 0.5),
            3,
        ),
        "sensor_ax_peak_power": round(
            profile["power"] ** 2 + rng.normal(0, 0.05),
            4,
        ),
        "sensor_ay_peak_power": round(
            profile["power"] ** 2 * 1.1 + rng.normal(0, 0.05),
            4,
        ),
        "sensor_az_peak_power": round(
            profile["power"] ** 2 * 0.4 + rng.normal(0, 0.03),
            4,
        ),
        # sensor gyro spectrum (lower amplitude variants)
        "sensor_gx_peak_hz": round(
            profile["sensor_ax"] * 0.6 + rng.normal(0, j),
            3,
        ),
        "sensor_gy_peak_hz": round(
            profile["sensor_ay"] * 0.6 + rng.normal(0, j),
            3,
        ),
        "sensor_gz_peak_hz": round(
            profile["sensor_az"] * 0.6 + rng.normal(0, j * 0.5),
            3,
        ),
        "sensor_gx_peak_power": round(
            profile["power"] * 0.3 + rng.normal(0, 0.03),
            4,
        ),
        "sensor_gy_peak_power": round(
            profile["power"] * 0.3 + rng.normal(0, 0.03),
            4,
        ),
        "sensor_gz_peak_power": round(
            profile["power"] * 0.15 + rng.normal(0, 0.02),
            4,
        ),
        # fused display
        "fused_dominant_freq_hz": round(
            (profile["vision_dx"] + profile["sensor_ax"]) / 2
            + rng.normal(0, j),
            3,
        ),
        "fusion_confidence": round(
            min(1.0, max(0.0, profile["conf"] + rng.normal(0, 0.04))),
            3,
        ),
        # rolling prediction

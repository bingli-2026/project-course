...
class MS6DSVI2CReader:
    ...
    def setup_default(self) -> None:
        """Configure the sensor for the 400 Hz Orange Pi baseline."""
        ...
        self.set_odr(REG_CTRL1, ODR_HA02_400HZ)
        self.set_odr(REG_CTRL2, ODR_HA02_400HZ)
    ...
    def estimate_rate_hz(self, *, sample_count: int = 2000) -> float:
        """Estimate the maximum host-side loop rate with burst reads."""
    ...
    def set_odr(self, register: int, encoded_odr: int) -> None:
        """Set CTRL1/CTRL2 ODR and HAODR selector from one encoded value."""

        self.update_bits(register, 0x0F, encoded_odr)
        haodr_sel = (encoded_odr >> 4) & 0x0F
        if haodr_sel:
            self.update_bits(REG_HAODR_CFG, 0x03, haodr_sel)

...
- sensor branch: 3-axis vibration spectrum
...
- `[analyze_video_stream.py](/home/davisye/test/test-motion-amplifier/scripts/analyze_video_stream.py)`
...
These are the fields we should target when the 3-axis sensor branch is added.
...
### Axis-Wise Time-Domain Features
...
| `sensor_ax_kurtosis` | float | Kurtosis of x axis |
| `sensor_ay_kurtosis` | float | Kurtosis of y axis |
| `sensor_az_kurtosis` | float | Kurtosis of z axis |

### Axis-Wise Spectrum Features
...
- sensor branch extraction: pending
...
- build a matching `sensor` sliding-window extractor that outputs the same identity fields plus `sensor_*` features

...
def align_feature_dict(
    features: dict[str, float], feature_columns: Iterable[str]
) -> pd.DataFrame:
...
def visual_vibration_features_from_frames(
    ...
    if len(frames) < 8:
        raise ValueError(
            "Not enough camera frames captured for visual vibration analysis."
        )
    ...
    if corners is None or len(corners) < min_tracks:
        raise ValueError(
            "Not enough stable visual corners in ROI. Adjust ROI or lighting."
        )
    ...
    for frame in frames[1:]:
        next_pts, status, _ = cv2.calcOpticalFlowPyrLK(
            prev, frame, current, None, **lk_params
        )
    ...
    features.update(
        _spectrum_features("vision_dx", dx, analysis_fps, min_frequency, max_frequency)
    )
    features.update(
        _spectrum_features("vision_dy", dy, analysis_fps, min_frequency, max_frequency)
    )
    return features
...
def vibration_features_from_samples(
    ...
    features: dict[str, float] = {
        ...
        "sensor_window_duration_s": float(window_seconds)
        if window_seconds
        else float(data.shape[0] / sample_rate_hz),
        "imu_sample_count": float(data.shape[0]),
    }

    for axis_index, axis_name in enumerate(axis_names[:axis_count]):
        _add_signal_features(
            features, f"sensor_{axis_name}", data[:, axis_index], sample_rate_hz
        )

    if axis_count >= 3:
        magnitude = np.sqrt(np.sum(data[:, :3] ** 2, axis=1))
        _add_signal_features(
            features, "sensor_accel_magnitude", magnitude, sample_rate_hz
        )
...
def vibration_features_from_i2c_samples(
    ...
    features: dict[str, float] = {
        ...
        "sensor_window_duration_s": float(window_seconds)
        if window_seconds
        else float(len(accel) / sample_rate_hz),
        "imu_sample_count": float(len(accel)),
    }

    for axis_index, axis_name in enumerate(["ax", "ay", "az"]):
        _add_signal_features(
            features, f"sensor_{axis_name}", accel[:, axis_index], sample_rate_hz
        )
    ...
    if gyro_samples:
        ...
        for axis_index, axis_name in enumerate(["gx", "gy", "gz"]):
            _add_signal_features(
                features, f"sensor_{axis_name}", gyro[:, axis_index], sample_rate_hz
            )
...
def _validate_roi(
    ...
    if (
        ...
        raise ValueError(
            f"ROI {roi} is outside frame bounds {(frame_width, frame_height)}."
        )
    return x, y, w, h
...
def _spectrum_features(
    ...
    return {
        ...
        f"{prefix}_spectral_entropy": float(
            -np.sum(probability * np.log2(probability + 1e-12))
        ),
    }
...
def _add_signal_features(
    ...
    features[f"{prefix}_crest_factor"] = (
        float(np.max(np.abs(values)) / rms) if rms > 0 else 0.0
    )
    if std > 0:
    ...
    if len(values) >= 4 and sample_rate_hz > 0:
        ...
        if total_power > 0:
            ...
            features[f"{prefix}_spectral_centroid_hz"] = float(
                np.sum(freqs * power) / total_power
            )

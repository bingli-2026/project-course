...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run independent realtime PCA fault detection."
    )
    parser.add_argument(
        "--visual", action="store_true", help="Enable camera visual PCA branch."
    )
    parser.add_argument(
        "--vibration", action="store_true", help="Enable vibration serial PCA branch."
    )
    ...
    parser.add_argument(
        "--port",
        help="Serial port for vibration module, for example COM3 or /dev/ttyUSB0.",
    )
...
def _visual_loop(
    ...
    while not stop_event.is_set():
        ...
        _print_result(
            "visual", output.scores[0], output.threshold, output.predictions[0]
        )
...
def _vibration_loop(
    ...
    while not stop_event.is_set():
        ...
        output = detector.predict(row)
        _print_result(
            "vibration", output.scores[0], output.threshold, output.predictions[0]
        )
        if save_normal_candidates:
            _save_candidate_if_confident(
                root=root,

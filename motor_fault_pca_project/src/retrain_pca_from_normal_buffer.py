...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Periodically retrain one PCA branch from high-confidence normal windows."
        )
    )
    ...
    if len(df) < args.min_windows:
        raise ValueError(
            "Not enough normal candidates for retraining: "
            f"{len(df)} < {args.min_windows}."
        )
    ...
    if len(normal_features) < args.min_windows:
        raise ValueError(
            "Not enough labeled normal rows after filtering: "
            f"{len(normal_features)} < {args.min_windows}."
        )

...
基于既定硬件链路（YUYV 640x480@400fps 摄像头 USB 直连香橙派 + 六轴传感器 400Hz 采样），实现双模态频率特征提取、特征级融合与状态识别。融合主线严格遵循 `doc/feature_schema.md`：先生成 `vision_*` 与 `sensor_*` 分轴窗口特征，再按 `sample_id + window_index` 拼接为统一向量，输入 RF/XGBoost/SVM 分类。前端数据大屏展示任务状态、频谱曲线、关键特征和分类结果。
...
**Constraints**: 摄像头固定 YUYV 640x480@400fps；六轴采样目标 400Hz；不引入复杂深度学习；硬件依赖测试不纳入 CI 全量执行  
...
- 摄像头：`YUYV`，`640x480`，`400fps`，通过 UVC/V4L2 从香橙派采集。
- 六轴（ax/ay/az + gx/gy/gz）：目标采样率 `400Hz`；当前边缘端优先使用香橙派侧稳定轮询 / 采样实现，不再追求 1kHz+。
...
- 输入流量 `400 * 34 = 13600 B/s`（约 13.3 KiB/s）
- `921600 bps / 10 = 92160 B/s`（8N1 理论字节率）
- 占用率约 `15%`，剩余约 `85%` 裕量

结论：921600 bps 远高于当前主线需求，串口不再是演示阶段瓶颈，但仍需要开启缓冲和丢包计数监控。
...
- 拟合窗口大小：`4.0s`（约 1600 个 IMU 点），兼顾抗抖动能力与漂移跟踪速度。
...
- **默认（部署基线）**：`window_size_s=0.5`，`window_hop_s=0.25`
- **可选（稳定展示模式）**：`window_size_s=1.0`，`window_hop_s=0.5`

默认值采用 `0.5/0.25`，是因为当前设备目标特征频率约 `20Hz`，该窗口长度既保留足够周期数，也降低边缘端数据量与显示抖动；`1.0s` 模式作为大屏低抖动展示配置。
...
| 摄像头帧中断 | 帧间时间差监控 | `> 3 * (1/400)s` | 标记 `camera_frame_gap`，触发相机流重启；重启失败则任务失败 |
